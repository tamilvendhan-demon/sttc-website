    ...
    <section className="bg-[#f6f0de] py-20">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-3 text-balance text-3xl font-semibold text-[#0b3733] sm:text-4xl">Frequently asked questions</h2>
        </div>
        <div className="mt-12 space-y-4">
          {faqs.map((item) => (
            <details key={item.question} className="group rounded-[22px] border border-[#d8c892] bg-white p-6 shadow-[0_16px_35px_rgba(11,55,51,0.04)]" open={item.question === faqs[0].question}>
              <summary className="cursor-pointer list-none text-lg font-semibold text-[#0b3733]">
                {item.question}
              </summary>
              <p className="mt-3 text-sm leading-7 text-[#4a473d]">{item.answer}</p>
            </details>
          ))}
        </div>
      </div>
